...
| `disable-model-invocation` | `true` **only** for user-action-only skills (`merge-pr`, `update`). Everything else stays model-invocable — `/ship`'s step subagents invoke skills via the Skill tool. |
