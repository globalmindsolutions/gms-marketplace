# acs:create-architecture — Codex CLI skill entry point (MAR-5, D1 Option B, shape a)
...
```bash
echo '{"cwd":"'"$PWD"'","tool_input":{"skill":"acs:create-architecture"}}' | python3 "$ACS_PLUGIN_ROOT/hooks/scripts/dispatch.py" pre
```

Coordinator body follows — unreachable if the shim above exits non-zero.

This is the Codex CLI entry point for the `/acs:create-architecture` skill.
